...
class EnhancedCompressionModel(ScaleHyperprior):
    ...
    def __init__(self, N=128, M=192, **kwargs):
        ...
        self.context_model = ContextModel(M)
        self.scale_refinement = ScaleRefinement(M)
        self.entropy_parameters = EntropyParameterNet(M)
    ...
    def forward(self, x):
        ...
        scales_hat = self.scale_refinement(scales_hat, y)
        ...
        means_hat, scales_hat = self.entropy_parameters(scales_hat, context)

        # -----------------------------
        # PERMANENT ENTROPY FIX
        # -----------------------------
        scales_hat = torch.clamp(scales_hat, min=1e-3, max=50.0)
